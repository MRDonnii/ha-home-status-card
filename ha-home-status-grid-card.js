...
class HaHomeSummaryCard extends HTMLElement {
  ...
  _render() {
    ...
    if (!this._rendered) {
      this.shadowRoot.innerHTML = `<style>
        :host{display:block;height:100%}.card{--summary-status:var(--success-color,#20e3a2);box-sizing:border-box;display:flex;flex-direction:column;height:100%;position:relative;overflow:hidden;padding:18px;border:0;border-left:3px solid var(--summary-status);border-radius:22px;background:linear-gradient(145deg,color-mix(in srgb,var(--white,#fff) 5%,transparent),transparent 42%),var(--ha-card-background,var(--surface,#15191f));box-shadow:0 12px 28px color-mix(in srgb,var(--black,#000) 22%,transparent);color:var(--primary-text-color,#fff)}
        ...
        .utilities{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-bottom:13px}.utility{--accent:var(--success-color,#20e3a2);position:relative;min-width:0;padding:13px 13px 12px 15px;border:0;border-left:3px solid var(--accent);border-radius:16px;background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 9%,transparent),color-mix(in srgb,var(--black,#000) 12%,transparent));overflow:hidden}.utility-head{display:flex;align-items:center;gap:7px;color:var(--secondary-text-color,#a7b2c2);font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.08em}.utility-head ha-icon{width:17px;height:17px;--mdc-icon-size:17px;color:var(--accent)}.use{display:flex;align-items:baseline;gap:4px;margin-top:7px}.use b{font-size:22px;line-height:1}.use span{font-size:10px;color:var(--secondary-text-color,#a7b2c2)}.cost{margin-top:7px;padding-top:7px;border-top:1px solid color-mix(in srgb,var(--accent) 22%,transparent);font-size:11px;color:var(--secondary-text-color,#a7b2c2)}.cost b{float:right;color:var(--primary-text-color,#fff);font-size:13px}.cost small{font-size:9px}
    ...
    const utility=(sel,icon,label,value,unit,cost,valid,estimated=false)=>{const tile=this.shadowRoot.querySelector(sel);tile.style.setProperty("--accent",valid?"var(--success-color,#20e3a2)":"var(--error-color,#ef4444)");tile.innerHTML=`<div class="utility-head"><ha-icon icon="${icon}"></ha-icon>${label}</div><div class="use"><b>${value}</b><span>${unit}</span></div><div class="cost">Månedspris ${estimated?'<small>(est.)</small>':''}<b>${cost} kr</b></div>`;};
