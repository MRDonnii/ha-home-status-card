...
class HaHomeStatusCard extends HTMLElement {
  ...
  render() {
    ...
    </style><button class="item" style="--accent:${item.color}" aria-label="${item.name || item.type}">${this.decoration(item)}<div class="value">${item.value}</div><div class="meter">${[1, 2, 3, 4, 5].map((n) => `<i class="seg ${n <= item.meter ? "on" : ""}"></i>`).join("")}</div><div class="detail">${item.detail || "&nbsp;"}</div><div class="label">${item.label}</div><ha-icon class="bg-icon" icon="${item.icon || "mdi:information-outline"}"></ha-icon></button>`;
    this.shadowRoot
      .querySelector(".item")
